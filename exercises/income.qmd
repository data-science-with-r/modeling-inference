...
The variable `income` in our dataset is stored as a `fct`, with the first level being `<=50K`, and second level being `>50K`. Because of this structure, we are going to be modeling the log-odds/probability of `>50K`
...
predict(income_model, age_predict, type = "response") 
```
