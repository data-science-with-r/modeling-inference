---
title: Predicting cholesterol with simple linear regression
...
webr:
  ...
  cell-options:
    autorun: false
  resources:
    - https://raw.githubusercontent.com/data-science-with-r/modeling-inference/refs/heads/main/exercises/data/cholesterol.csv
---
...
cholesterol <- read_csv("https://raw.githubusercontent.com/data-science-with-r/modeling-inference/refs/heads/main/exercises/data/cholesterol.csv")
...
The original data for this exercise are located in the [**faraway**](https://cran.r-project.org/web/packages/faraway/index.html) package, but for this exercise you will load the data from a CSV file.
...
Note: For this exercise, there are some missing data. In your output, you may see a R Warning saying that some rows in the dataset having missing values. We will ignore this warning for the purpose of this exericse.
...
```{r}
lm_fit <- linear_reg() |>
  fit(chol ~ weight, data = cholesterol)
```
...
We are going to re-estimate our line of best fit for each of these added observations. To create a new observation, please fill in the `____` with the appropriate values. Next, add this value to the existing data set using the `bind_rows()` function.
...
cholesterol_new <- bind_rows(cholesterol, new_obs)
...
```{webr}
lm_fit <- linear_reg() |> 
  fit(chol ~ weight, data = cholesterol_new)
...
## Workign with categorical predictors
