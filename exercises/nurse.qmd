---
title: Quantifying uncertanity in the ICU
...
gilbert <-
  tibble(
    outcome = c(rep("died", 74), rep("no-death", 1567)),
    working = c(
      rep("gilbert", 40),
      rep("no-gilbert", 34),
      rep("gilbert", 217),
...
The dataset you will be working with is called `gilbert`. The data are already pre-loaded for you! Simply run the code below to get started. The data dictionary can be seen below.
...
Our observational units are whom or what we take the data off of. In this case, data are collected from individual shifts. Both variables that we are working with are categorical variables.
:::
...
You have now created 1000 simulated difference in proportions under the assumption of $H_0: \pi_\text{no-gilbert} - \pi_\text{gilbert} = 0$. 
...
```{r}
visualize(null_dist) +
  shade_p_value(-0.13, direction = "less")
```

We notice that, out of 1000 samples generated under the assumption of the null hypothesis, we observed 0 samples as small, or even smaller than -0.13. This would indicate that our p-value is very very small.
...
This process is called bootstrap re-sampling.
...
boot_dist <- gilbert |>
  specify(response = ____, explanatory = ____, success = "___") |> # specify variables 
  generate(reps = ____, type = "bootstrap") |> # use bootstrap with appropriate number of reps
  calculate(stat = "_____", order = c("_____", "_____")) # specify statistic and order of subtraction
```
...
boot_dist <- gilbert |>
  specify(response = outcome, explanatory = working, success = "died") |> # specify variables
  generate(reps = 1000, type = "bootstrap") |> # use bootstrap with appropriate number of reps
  calculate(stat = "diff in props", order = c("no-gilbert", "gilbert")) # specify statistic and order of subtraction
```
...
Now, let's plot our simulated difference in sample proportions below using `
geom_histogram()
`. Add a vertical line at your statistic using `
geom_vline()
` to help visualize that the distribution is centered at our statistic.
:::
...
```{r}
boot_dist |>
  summarize(lower = quantile(stat, 0.10), upper = quantile(stat, 0.90))
```
...
- Depending on the methodology, the simulation process will be different, dictating where our sampling distribution is centered.
